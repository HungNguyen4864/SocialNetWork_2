...
class FacebookGroupPostCrawl:
    ...
    def save_comments_to_excel(self, detailpost):
        """
        Save comment data to an Excel file.

        Args:
            detailpost (list): A list of tuples, each containing post ID and its comments.
        """
        try:
            file_name = f"ex_comments.xlsx"
            # Create a DataFrame from the detailpost list
            df = pd.DataFrame(detailpost, columns=["post_id", "list_comments"])
            # Save the DataFrame to an Excel file
            df.to_excel(file_name, index=False)
            print(f"Comment data saved to {file_name}")
        except Exception as e:
            # Print any errors that occur
            print(f"Error saving comments to Excel: {e}")
    def save_post_to_excel(self, postlist: list) -> None:
        """
        Save post data to an Excel file.

        Args:
            postlist (list): A list of tuples, each containing post ID, user ID, and post content.
        """
        try:
